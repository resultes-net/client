<script lang="ts">
	...
	import { clampValue, parseAndClampInputValue } from '$lib/utils';
...
<div class="m-2 p-2">
	<div class="grid grid-cols-[--input-grid-cols] items-center gap-y-[--input-gap-y]">
		<label for="setpoint-temperatures-input-mode">{$t('common.temperatureInputMode')}</label>
		<select id="setpoint-temperatures-input-mode" class="select" bind:value={mode}>
			<option value="absolute">{$t('common.setpointTemperaturesInputModeAbsolute')}</option>
			<option value="relative">{$t('common.setpointTemperaturesInputModeRelative')}</option>
		</select>
	</div>

	<hr class="mt-2 mb-3 mr-1" />
